...
class CertificateParser:
    ...
    def export_certificate(self, cert_data: Dict, format_type: CertificateFormat = CertificateFormat.PEM) -> bytes:
        """
        Export certificate in the specified format.
        Reads from the original file on disk and converts as needed.
        Supports PEM, DER, PKCS7_PEM, PKCS12 (no private key), COSE, CWT.
        """
        file_path = cert_data.get('file_path', '')

        if format_type in (CertificateFormat.PEM, CertificateFormat.PKCS10_PEM):
            if file_path and os.path.exists(file_path):
                with open(file_path, 'rb') as f:
                    raw = f.read()
                detected = self.detect_format(raw, Path(file_path).suffix)
                if detected == CertificateFormat.DER:
                    cert = x509.load_der_x509_certificate(raw)
                    return cert.public_bytes(serialization.Encoding.PEM)
                return raw
            raise FileNotFoundError(f"Certificate file not found or not on disk: {file_path}")

        elif format_type == CertificateFormat.DER:
            if file_path and os.path.exists(file_path):
                with open(file_path, 'rb') as f:
                    raw = f.read()
                detected = self.detect_format(raw, Path(file_path).suffix)
                if detected == CertificateFormat.PEM:
        ...
        raise NotImplementedError(f"Export to {format_type.value} not implemented")
    ...
    def _extract_certificate_info(self, cert: x509.Certificate, file_path: str) -> Dict:
        ...
        # Handle both cryptography <42 (naive UTC) and >=42 (timezone-aware) APIs
        not_valid_after = (
    ...
    def _determine_cert_type(self, cert_info: Dict) -> str:
        ...
        # CA certificates: key_cert_sign or crl_sign set in KeyUsage
        if key_usage.get('key_cert_sign') or key_usage.get('crl_sign'):
            return 'ca'
        ...
        if 'serverAuth' in extended_key_usage:
            ...
            return 'email'
        elif 'ocspSigning' in extended_key_usage:
            return 'ocsp'
        elif 'timeStamping' in extended_key_usage:
            return 'timestamping'
        else:
            return 'unknown'
    ...
    def _categorize_issuer(self, issuer_cn: str) -> str:
        ...
        if "let's encrypt" in issuer_cn_lower or 'letsencrypt' in issuer_cn_lower:
            return 'letsencrypt'
        elif any(k in issuer_cn_lower for k in ('digicert', 'rapidssl', 'geotrust', 'thawte')):
            return 'digicert'
        elif any(k in issuer_cn_lower for k in ('sectigo', 'comodo', 'comodoca', 'usertrust', 'addtrust')):
            return 'sectigo'
            ...
            return 'cloudflare'
        elif 'microsoft' in issuer_cn_lower:
            return 'microsoft'
        elif 'google' in issuer_cn_lower:
            return 'google'
        elif not issuer_cn_lower:
            return 'unknown'
        else:
            # Non-empty but unrecognised issuer — treat as private/internal CA
            return 'private'

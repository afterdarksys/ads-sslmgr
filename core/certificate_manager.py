...
class CertificateManager:
    ...
    def create_cose_certificate(self, subject: dict, algorithm: str = 'ES256',
                                embed_x509: bool = False, user_id: int = None) -> Dict:
        """Create a COSE_Sign1 token from *subject* info and store it as a Certificate record.

        The token is signed with an ephemeral EC P-256 key (ES256).  The raw
        COSE bytes are written to a file in the configured cache directory and
        the file path is recorded in the Certificate table.

        Returns {'success': True, 'certificate_id': int} on success.
        """
        try:
            from .certificate_parser import CertificateFormat

            # Build synthetic cert_data for the COSE exporter
            now = datetime.utcnow()
            cert_data = {
                'common_name': subject.get('common_name', 'COSE Certificate'),
                'serial_number': str(int(now.timestamp())),
                'issuer': {'common_name': 'SSL Certificate Manager'},
                'subject': subject,
                'not_valid_before': now.isoformat(),
                'not_valid_after': (now + timedelta(days=365)).isoformat(),
                'signature_algorithm': algorithm,
                'days_until_expiry': 365,
                'format': 'cose',
            }

            cose_bytes = self.parser._export_to_cose(cert_data)

            # Write to cache directory
            cache_dir = Path(self.config.get('directories', {}).get('cache', './cache'))
            cache_dir.mkdir(parents=True, exist_ok=True)
            file_path = cache_dir / f"cose_{cert_data['serial_number']}.cose"
            file_path.write_bytes(cose_bytes)

            # Store as a Certificate record
            session = self.db_manager.get_session()
            try:
                ...
                cert = Certificate(
                    ...
                    days_until_expiry=365,
                    signature_algorithm=algorithm,
                    certificate_type='cose',
                    issuer_category='cose',
                    issuer_info={'common_name': 'SSL Certificate Manager'},
    ...
    def create_cwt_token(self, issuer: str, subject: str, audience: str,
                         expiry_days: int = 365, user_id: int = None) -> Dict:
        """Create a signed CWT (CBOR Web Token) and store it as a Certificate record.
        ...
        try:
            ...
            cert_data = {
                ...
                'not_valid_before': now.isoformat(),
                'not_valid_after': (now + timedelta(days=expiry_days)).isoformat(),
                'signature_algorithm': 'ES256',
                'days_until_expiry': expiry_days,
                'format': 'cwt',
            }

            cwt_bytes = self.parser._export_to_cwt(cert_data)

            cache_dir = Path(self.config.get('directories', {}).get('cache', './cache'))
            cache_dir.mkdir(parents=True, exist_ok=True)
            file_path = cache_dir / f"cwt_{cert_data['serial_number']}.cwt"
            file_path.write_bytes(cwt_bytes)
            ...
            try:
                ...
                cert = Certificate(
                    ...
                    days_until_expiry=expiry_days,
                    signature_algorithm='ES256',
                    certificate_type='cwt',
                    issuer_category='cwt',
                    issuer_info={'common_name': issuer},
                    subject_info={'common_name': subject, 'audience': audience},
                    is_active=True,
                )
                session.add(cert)
                session.commit()
    ...
    def get_cose_certificate_details(self, cert_id: int) -> Dict:
        """Return details for a COSE or CWT certificate stored by *cert_id*."""
        session = self.db_manager.get_session()
        try:
            from database.models import Certificate
            ...
            if cert.certificate_type not in ('cose', 'cwt'):
                return {'success': False, 'error': 'Certificate is not a COSE/CWT type'}
            return {
                'success': True,
                'certificate': {
                    'id': cert.id,
